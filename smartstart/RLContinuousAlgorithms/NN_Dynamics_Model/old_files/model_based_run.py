...
def load_yaml(yaml_file):
    ...
    yaml_path = os.path.abspath('yaml_files/' + yaml_file + '.yaml')
    ...
    return which_agent, use_threading,\
    num_rollouts_train, num_rollouts_val, num_fc_layers,\
    depth_fc_layers, batchsize, lr, nEpoch, fraction_use_new,\
    horizon, num_control_samples, num_aggregation_iters,\
    num_trajectories_for_aggregation, rollouts_forTraining,\
    make_aggregated_dataset_noisy, make_training_dataset_noisy,\
    noise_actions_during_MPC_rollouts, dt_steps, steps_per_episode,\
    steps_per_rollout_train, steps_per_rollout_val, min_rew_for_saving,\
    visualize_True, visualize_False
...
def model_based_run(seed=0,
    ...
    save_dir = make_save_directories(run_num)
    ...
    with tf.Session(config=config) as sess:
        ...
        if (use_existing_training_data):
            ...
            states, controls, _, _ = perform_rollouts(random_policy, num_rollouts_train, steps_per_rollout_train,
                                                      visualize_False, CollectSamples, env, dt_steps, dt_from_xml)
            ...
            states_val, controls_val, _, _ = perform_rollouts(random_policy, num_rollouts_val, steps_per_rollout_val,
                                                              visualize_False, CollectSamples, env, dt_steps,
                                                              dt_from_xml)
        ...
        might_render = False
        if (visualize_MPC_rollout or might_render):
            might_render = True
        if (might_render):
        ...
        dyn_model = Dyn_Model(inputSize, outputSize, sess, lr, batchsize, num_fc_layers, depth_fc_layers, mean_x,
                              mean_y, mean_z, std_x, std_y, std_z, tf_datatype, print_minimal)

        # create mpc controller
        mpc_controller = MPCController(env, dyn_model, horizon, which_agent, steps_per_episode, dt_steps,
                                       num_control_samples,
                                       mean_x, mean_y, mean_z, std_x, std_y, std_z, actions_ag, print_minimal, x_index,
                                       y_index,
        ...
        while (counter_agg_iters < num_aggregation_iters):
            ...
            if (use_existing_dynamics_model):
            ...
            if (perform_forwardsim_for_vis):
                if (not (print_minimal)):
                    print("\n#####################################")
                    print(
                        "Performing a forward sim of the learned model. using pre-saved dataset. just for visualization")
                    print("#####################################\n")
                ...
                np.save(save_dir + '/saved_forwardsim/forwardsim_states_true_' + str(counter_agg_iters) + '.npy',
                        forwardsim_x_true)
                np.save(save_dir + '/saved_forwardsim/forwardsim_states_pred_' + str(counter_agg_iters) + '.npy',
                        forwardsim_x_pred)
            ...
            horiz_penalty_factor, forward_encouragement_factor, heading_penalty_factor, desired_snake_headingInit = get_trajfollow_params(
                which_agent, desired_traj_type)
            ...
            for rollout_num in range(num_trajectories_for_aggregation):
                ...
                if (which_agent == 2):
                    ...
                    starting_state = copy.deepcopy(starting_observation)
                else:
                    starting_observation = env.reset()
                    starting_state = copy.deepcopy(starting_observation)
                # start swimmer heading in correct direction
                ...
                desired_x = make_trajectory(desired_traj_type, starting_observation_NNinput, x_index, y_index,
                                            which_agent)
                ...
                if (noise_actions_during_MPC_rollouts):
                    ...
                    curr_noise_amount = 0
                resulting_x, selected_u, ep_rew, _ = mpc_controller.perform_rollout(starting_state,
                                                                                    starting_observation,
                                                                                    starting_observation_NNinput,
                                                                                    desired_x,
                                                                                    follow_trajectories,
                                                                                    horiz_penalty_factor,
                                                                                    forward_encouragement_factor,
                                                                                    heading_penalty_factor,
                                                                                    noise_actions_during_MPC_rollouts,
                                                                                    curr_noise_amount)
            ...
            if (visualize_MPC_rollout):
                input("\n\nPAUSE BEFORE VISUALIZATION... Press Enter to continue...")
                for vis_index in range(num_trajectories_for_aggregation):
                    visualize_rendering(starting_states[vis_index], selected_multiple_u[vis_index], env, dt_steps,
            ...
            np.save(save_dir + '/saved_trajfollow/pred_iter' + str(counter_agg_iters) + '.npy',
                    np.array(resulting_multiple_x))
        ...
        if (num_rollouts_save_for_mf > 0):
            ...
            num_saved = 0
            rollout_num = 0
            while (num_saved < num_rollouts_save_for_mf):
                if (not (print_minimal)):
...
if __name__ == '__main__':
    model_based_run('swimmer_forward')