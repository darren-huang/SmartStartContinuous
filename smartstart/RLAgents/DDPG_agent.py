...
class DDPG_agent(RLAgent, metaclass=ABCMeta):
    ...
    def get_state_value(self, state):
        ...
        # return q_value
        return self.critic.predict_target(state, self.actor.predict_target(state),20)
    ...
    def observe(self, state, action, reward, new_state, done):
        if self.train_indicator:
            ...
            if self.replay_buffer.size() > self.MINIBATCH_SIZE:
                ...
                target_q = self.critic.predict_target(s2_batch, self.actor.predict_target(s2_batch), 20)
